...
if(mode==='stop') {
 const infoPath=resolve(process.argv[3]||'');if(!infoPath.startsWith(resolve(root,'artifacts/a2/runs')+'/')&&!infoPath.startsWith(resolve(root,'artifacts/a2/runs')+'\\'))throw Error('Not an A2 artifact')
 const info=JSON.parse(readFileSync(infoPath,'utf8'))
 if(!/^a2-live-[\dTZ-]+$/i.test(info.prefix))throw Error('Invalid container prefix')
 if(!resolve(info.work).startsWith(resolve(root,'tools/dev-harness/.work/a2-live-')))throw Error('Invalid workspace')
 ...
 const reports=resolve(info.work,'target/surefire-reports');if(existsSync(reports))for(const file of readdirSync(reports).filter(p=>p.includes('A2HttpIT')&&(p.endsWith('.txt')||p.endsWith('.xml'))))copyFileSync(resolve(reports,file),resolve(info.out,file))
 for(const role of ['frontend','backend','redis','mysql'])cmd(['rm','-f',info.prefix+'-'+role])
 cmd(['network','rm',info.prefix+'-net']);info.cleanedUp=true;writeFileSync(infoPath,JSON.stringify(info,null,2));console.log('A2 live stopped; backend exit '+info.backendExitCode);process.exitCode=info.backendExitCode; 
} else if(mode==='start') {
 const stamp=new Date().toISOString().replace(/[:.]/g,'-'),prefix='a2-live-'+stamp
 const work=resolve(root,'tools/dev-harness/.work',prefix),out=resolve(root,'artifacts/a2/runs',stamp+'-live')
 ...
 '-e','A2_ISOLATED=true','-e','A2_BROWSER_HOLD=true','-e','A2_REVIEW_DELAY_MS=1200','-e',`A2_TEST_DB_URL=jdbc:mysql://${prefix}-mysql:3306/ai_passage_creator?useUnicode=true&characterEncoding=UTF-8`,'-e',`A2_REDIS_HOST=${prefix}-redis`,'-e',`A2_IMAGE_ORIGIN=http://127.0.0.1:${frontPort}`,maven,'mvn','-s','/work/a0-maven-settings.xml','-o','-B','-Dtest=A2HttpIT','test'])
 const backendPort=frontPort
 const info={prefix,work,out,frontend:`http://127.0.0.1:${frontPort}`,backend:`http://127.0.0.1:${backendPort}`,dependencySnapshot:prior,images:{mysql,redis,maven,node},cleanedUp:false}
 writeFileSync(resolve(out,'live.json'),JSON.stringify(info,null,2));console.log('A2 live environment: '+resolve(out,'live.json'));console.log('Wait for '+resolve(work,'target/a2-http-ready.json')+' before browser validation')
